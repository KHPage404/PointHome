...
<div class="container text-center">
	<h1 style="text-align: center; font-family: 'SBAggroM';"><i class="bi bi-journal-text"></i>자유게시판</h1>
	<br>
	<!-- 검색 기능 -->
	...
	<div id="filterDiv" style="text-align: right">
		<p style="font-family: 'SBAggroM';">정렬기준 / 
		<select class="array" id="filter" name="filter"
			onchange="filterSelect()" style="border-radius: 1em;">
			<c:choose>
		...
		<table id="listTable" class="table table-hover table-sm text-center"
			...
			<thead>
				<tr style="font-family: 'SBAggroM'; font-size:15px;">
					<th>글번호</th>
					<th>제목</th>
					<th>닉네임</th>
					<th>조회수</th>
			...
			<c:forEach var="board" items="${list }">
				<tr
					...
					<td style="height: 80px;">${board.FREEBOARD_NO }</td>
					<td class="text-start " style="font-weight: bold; font-size: 20px"><a
		...
		<c:if test="${not empty login and login}">
			...
			<div class="float-end mb-3">
				<a href="./write"><button id="btnWrite" class="btn btn-info" style="font-family: 'SBAggroM';">글쓰기</button></a>
			</div>
		...
		<ul class="pagination justify-content-center">
			...
			<c:if test="${paging.curPage ne 1 }">
				<li class="page-item"><a class="page-link"
					href="./list?filter=${filter}&searchType=${searchType}&keyword=${keyword}">&larr;
						처음</a></li>
			</c:if>
			...
			<c:if test="${paging.curPage ne paging.totalPage }">
				<li class="page-item"><a class="page-link"
					href="./list?curPage=${paging.totalPage }&filter=${filter}&searchType=${searchType}&keyword=${keyword}">마지막&rarr;
				</a></li>
			</c:if>
